(function () {
  ...
  const LANGUAGES = {
    ...
    english: {
      ...
      short: "英語",
      locale: "en-US",
      description: "中文教學場景提示，候選人需用英語自然表達。",
      mode: "scenario"
    }
  };
  ...
  async function renderTests() {
    ...
    for (const test of tests) {
      ...
      recordButton.disabled = attemptsLeft === 0;
      submitButton.disabled = !latest || Boolean(test.selectedAttemptId);
      retakeButton.disabled = attemptsLeft === 0 || Boolean(test.selectedAttemptId);
      ...
      retakeButton.addEventListener("click", () => setHint(`${lang.label} 可以重新录制，题目保持不变。`));
